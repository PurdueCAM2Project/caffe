...
class YoloLossLayer : public LossLayer<Dtype> {
 public:
  ...
  // bottom[0] stores the location predictions and confidence predictions
 ...
 protected:
  ...
  int num_;
  int k_; // number of anchors: could be derived from provided anchor boxes in prototxt
  int image_input_size_; // image input size
  int result_side_size_; // downsample by 32: for 416 is 13x13
  int num_bboxes_; // total number of bounding boxes
  int num_classes_;
  int num_gt_;
  int num_matches_;
  int loc_size_;
  int input_size_;
  vector<pair<float,float> >anchors_;
